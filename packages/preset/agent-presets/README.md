...
The shipped Web `standard`, `ptc`, and `cordis` presets include [explicit file delivery](../../client/ui-deliverables/README.md#explicit-deliveries). The `minimal` preset keeps its fixed shell-only coding configuration. The tool-free `learning` preset supplies a complete beginner Tutor persona without runtime workspace context; a learning App provides the course through logged Session messages.
