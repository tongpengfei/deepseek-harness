...
| `MarkdownText`, `MarkdownDelegateProvider`, `CodeBlock` | Untrusted GFM with TeX math, owner-delegated HTTP(S) navigation, and highlighted code. `CodeBlock` accepts `lineNumbers`, while `MarkdownText` forwards `codeLineNumbers` to every fenced code block; copied source excludes the gutter. `contentRef` exposes the stable source wrapper to an owner that uses it as a scrollport. Set `showHeader={false}` when the owner supplies its own language and copy toolbar. |
