...
| `MarkdownText`、`MarkdownDelegateProvider`、`CodeBlock` | 不可信 GFM 与 TeX 数学、owner 委托的 HTTP(S) 导航，以及高亮代码。`CodeBlock` 接受 `lineNumbers`，`MarkdownText` 则把 `codeLineNumbers` 传给每个围栏代码块；复制的源码不含行号栏。`contentRef` 向需要把稳定源码包装节点用作滚动区的 owner 提供该节点。调用方提供自己的语言与复制工具栏时，设置 `showHeader={false}`。 |
